...
def process_manifest(in_fn, out_fn, core_overrides_fn=None):
    manifest = None
    ...
    if not manifest:
        ...
        sys.exit(1)
    core_overrides = None
    if core_overrides_fn:
        if not os.path.exists(core_overrides_fn):
            print('Error: Core overrides file does not exist: {}'.format(core_overrides_fn))
            sys.exit(1)
        with open(core_overrides_fn, 'r') as core_overrides_fh:
            core_overrides = json.load(core_overrides_fh)
            if not core_overrides:
                print('Error: Failed to load core overrides file: {}'.format(core_overrides_fn))
                sys.exit(1)
...
""".lstrip())
        ...
        process_manifest_metadata(metadata, out_fh, core_overrides)
    return

def process_manifest_metadata(metadata, out_fh, core_overrides=None):
...
""".lstrip())
    process_manifest_item_by_sample_set(orderedSets, sets, out_fh, core_overrides)
...
def process_manifest_item_by_sample_set(orderedSets, sets, out_fh, core_overrides=None):
    hgTrackServerBySampleSetObj = {}
    navbarDescriptionsBySampleSetObj = {}
    formattedDescriptionsBySampleSetObj = {}
    availableOverriddenSampleSetObj = {}
    for orderedSet in orderedSets:
        ...
        if not hgTrackServerBySampleSetObj[orderedSet]:
            ...
            sys.exit(1)
        if core_overrides and orderedSet in core_overrides:
            core_overrides_data = core_overrides[orderedSet]
            if 'navbarDescription' in core_overrides_data:
                navbarDescriptionsBySampleSetObj[orderedSet] = core_overrides_data['navbarDescription']
            if 'hgTrackServer' in core_overrides_data:
                hgTrackServerBySampleSetObj[orderedSet] = core_overrides_data['hgTrackServer']
            availableOverriddenSampleSetObj[orderedSet] = {}
            availableOverriddenSampleSetTracks = core_overrides_data['tracks'] if 'tracks' in core_overrides_data else None
            if availableOverriddenSampleSetTracks:
                for track in availableOverriddenSampleSetTracks:
                    trackType = track['type'] if 'type' in track else None
                    if not trackType: continue
                    if trackType == 'epilogos':
                        trackAssembly = track['assembly'] if 'assembly' in track else None
                        if not trackAssembly: continue
                        trackModel = track['model'] if 'model' in track else None
                        if not trackModel: continue
                        trackComplexity = track['complexity'] if 'complexity' in track else None
                        if not trackComplexity: continue
                        if not availableOverriddenSampleSetObj[orderedSet].get(trackAssembly):
                            availableOverriddenSampleSetObj[orderedSet][trackAssembly] = {}
                        if not availableOverriddenSampleSetObj[orderedSet][trackAssembly].get(trackModel):
                            availableOverriddenSampleSetObj[orderedSet][trackAssembly][trackModel] = {}
                        if not availableOverriddenSampleSetObj[orderedSet][trackAssembly][trackModel].get(trackComplexity):
                            availableOverriddenSampleSetObj[orderedSet][trackAssembly][trackModel][trackComplexity] = []
                        trackGroup = track['group'] if 'group' in track else None
                        if not trackGroup: continue
                        availableOverriddenSampleSetObj[orderedSet][trackAssembly][trackModel][trackComplexity].append(trackGroup)
    ...
    availableOverriddenSampleSetString = json.dumps(availableOverriddenSampleSetObj, indent=OBJ_INDENT)
    out_fh.write(f"""
...
export const availableOverriddenSampleSet = {availableOverriddenSampleSetString};
""".lstrip())
    return
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print('Usage: {} <manifest_path> <output_path> [<core_overrides_path>]'.format(sys.argv[0]))
        sys.exit(1)
    manifest_input_path = sys.argv[1]
    manifest_output_path = sys.argv[2]
    manifest_core_overrides_path = sys.argv[3] if len(sys.argv) >= 4 else None
    ...
    process_manifest(manifest_input_path, manifest_output_path, manifest_core_overrides_path)