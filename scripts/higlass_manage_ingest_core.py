...
'''
The local_overrides object is used to store properties of the ingested tracks, as well as provide
...
def append_candidate_url_entry_to_local_overrides_obj(baseUploadsFn, candidateUrl):
    ...
    if sampleSet not in local_overrides:
        local_overrides[sampleSet] = {
            "tracks": [],
            "trackServer": localHgServerTrackUrl,
        }
    retrievalTimestamp = datetime.datetime.now().isoformat()
    local_overrides[sampleSet]['tracks'].append({
    ...
    for candidateUrl in candidateUrls:
        ...
        if baseUploadsFn and mediaStagingPath and mediaUploadsPath:
            ...
            if os.path.exists(mediaUploadsPath) and base_uploads_fn_exists_in_hg_manage_tilesets(baseUploadsFn):
                delete_staging_path(mediaStagingPath)
                append_candidate_url_entry_to_local_overrides_obj(baseUploadsFn, candidateUrl)
            else:
...
if __name__ == '__main__':
    ...
    if local_overrides:
        with open('manifest.local_overrides.json', 'w') as local_overrides_fh:
            json.dump(local_overrides, local_overrides_fh, indent=4)